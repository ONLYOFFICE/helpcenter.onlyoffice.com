...
const getSearchResults = async (locale, query, page, pageSize) => {
  ...
  const urls = endpoints.map(endpoint => `${CMSConfigAPI}/api/${endpoint}/?pagination[pageSize]=1000&pagination[page]=1&locale=${locale}&filters[$or][0][title][$contains]=${query}&filters[$or][1][content][$contains]=${query}&filters[$or][2][url][$contains]=${query.replace(/\s/g, '-')}`);
