...
const getLevel4Data = async (locale, category, url, preview ) => {
  ...
  if (localeArticleData?.data?.length === 0 && locale === "zh" && category === "docs") {
    ...
    if (englishArticleData?.data?.length > 0) {
       await translateText(englishArticleData.data[0].id, `article-${categorySlugPlural}`, locale, false);
       articleData = await getArticle(locale, category, url);
    }
  } else if (localeArticleData?.data?.length !== 0 && locale === "zh" && category === "docs") {
    await translateText(localeArticleData.data[0].id, `article-${categorySlugPlural}`, locale, true);
    articleData = await getArticle(locale, category, url);
  } else {
