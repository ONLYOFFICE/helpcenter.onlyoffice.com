...
`.replace(/\s+/g, "");
  ...
  const res = await fetch(
    `${CMSConfigAPI}/api/${category === "docs" ? "articles-docs" : `article-${category}s`}?locale=${locale}${params}${category !== "desktop" ? level3Params : ""}${category !== "desktop" && category !== "docspace" ? level4Params : ""}`,
  );
