...
const getLevel2Data = async (locale, category, url, preview) => {
  ...
  const level2Populate = category !== "desktop" ? `
...
` : "";
  ...
  if (localeArticleData?.data?.length === 0 && category === "integration" && locale === "ja") {
      const urlWithoutLocale = url.replace(new RegExp(`^/${locale}`, 'i'), '');
      const localeArticleParams = `
      &filters[url][$eq]=${urlWithoutLocale}
      ...
      `)}
    `.replace(/\s+/g, "");
      const englishArticleData = await fetch(`${CMSConfigAPI}/api/${articleApi}/?locale=en${localeArticleParams}${preview ? "&status=draft" : ""}`).then(res => res.json());
      if (englishArticleData?.data?.length > 0) {
        await translateText(englishArticleData.data[0].id, articleApiSingular, locale);
        articleData = await fetch(`${CMSConfigAPI}/api/${articleApi}/?locale=${locale}${articleParams}${preview ? "&status=draft" : ""}`).then(res => res.json());
      }
    } else {
  ...
  if (articleData?.data?.length) {
    articleData.data[0].article = true;
